...
async function main() {
  ...
  // Check if Firefox is installed before doing anything
  ...
  if (!(installedApps as any)[FIREFOX_PACKAGE]) {
    await fail("FIREFOX_NOT_INSTALLED");
    return;
  }

  // Start with the Launch stage (open Firefox + navigate to YouTube)
  await setStage(Stage.Launch);

  // Execute the stage's default handler (go home, then launch Firefox)
